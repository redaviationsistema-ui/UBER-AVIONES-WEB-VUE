...
function sidebarAircraftTypeLabel(aircraft = {}) {
  const normalized = String(aircraft.cabin || aircraft.category || aircraft.aircraft_category || '')
    .toLowerCase()
    .trim()
...
function aircraftCapacityValue(aircraft = {}) {
  const raw = String(aircraft.capacity || aircraft.passenger_capacity || '').match(/(\d+(?:\.\d+)?)/)
  return Number(raw?.[1] || 0)
}

function aircraftSpeedValue(aircraft = {}) {
  const speedKmh = Number(aircraft.speed_kmh || aircraft.speedKmh || aircraft.cruise_speed_kmh || 0)
...
function normalizeSidebarServiceLabel(value = '') {
  ...
  if (normalized.includes('bano') || normalized.includes('baño') || normalized.includes('lavatory'))
...
const aircraftSidebarTypeOptions = computed(() => {
  ...
  return [...counts.entries()]
    ...
    .sort((current, next) => next.count - current.count || current.label.localeCompare(next.label))
})
...
const sidebarFilteredAircraftOptions = computed(() => {
  ...
  return routeVisibleAircraftOptions.value.filter((aircraft) => {
    ...
    if (selectedServices.size && ![...selectedServices].every((service) => services.includes(service)))
      return false
...
const filteredAircraftOptions = computed(() => {
  const options = sidebarFilteredAircraftOptions.value.map((aircraft, index) => ({ aircraft, index }))
