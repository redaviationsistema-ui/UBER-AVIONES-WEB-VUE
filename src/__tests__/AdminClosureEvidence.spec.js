...
it('opens and closes modal with operation metadata', async () => {
  ...
  const wrapper = mount(Evidence, { props: { operationId: 10 } })
  await flushPromises()
  await wrapper.get('.closure-grid a').trigger('click')
...
it('shows only the checklist summary in compact mode', async () => {
  api.get.mockResolvedValue(payload(2))
  const wrapper = mount(Evidence, { props: { operationId: 10, compact: true } })
  await flushPromises()
  expect(wrapper.text()).toContain('2/3')
  expect(wrapper.text()).toContain('✓ Catering')
  expect(wrapper.text()).toContain('Pendiente · Cabina final')
  expect(wrapper.findAll('img')).toHaveLength(0)
  expect(wrapper.text()).not.toContain('catering_received.jpg')
})
it('downloads the real file with a readable filename', async () => {
  api.get.mockResolvedValue(payload(1))
  ...
  const click = vi.spyOn(HTMLAnchorElement.prototype, 'click').mockImplementation(function () {
    expect(this.download).toBe('Catering.jpg')
  })
  const wrapper = mount(Evidence, { props: { operationId: 10 } })
  await flushPromises()
  await wrapper.get('.closure-grid button').trigger('click')
  await flushPromises()
  expect(fetchMock).toHaveBeenCalledWith('https://storage.test/10/catering_received.jpg?signed=1')
