...
const roleOptions = [
  {
    id: 'provider',
    label: 'Proveedor',
    description: 'Flota, disponibilidad y respuesta operativa a solicitudes.',
    email: 'operador@privateflights.test',
  },
  {
    id: 'sobrecargo',
    label: 'Sobrecargo',
    description: 'Agenda, briefings, servicios e incidencias.',
    email: 'crew@privateflights.test',
  },
  {
    id: 'admin',
    label: 'Administrativo',
    description: 'Usuarios, alertas, ingresos y control ejecutivo.',
    email: 'admin@privateflights.test',
  },
]

const requestedRole = typeof route.query.role === 'string' ? route.query.role : 'provider'
const selectedRole = ref(
  ['provider', 'sobrecargo', 'admin'].includes(requestedRole) ? requestedRole : 'provider',
)
...
const activeRole = computed(
  () => roleOptions.find((item) => item.id === selectedRole.value) ?? roleOptions[0],
)

function applyRole(roleId) {
  selectedRole.value = roleId
  const preset = roleOptions.find((item) => item.id === roleId)

  if (preset && !form.email) {
...
<template>
  <main class="internal-login-page">
    <section class="internal-login-shell">
      <div class="login-card">
        <div class="login-copy">
          <p class="role-kicker">Acceso interno</p>
        ...
        <p class="terms">
          Al continuar, aceptas recibir información relacionada con operación aérea,
          seguimiento de servicios y comunicaciones de Sky Group.
        </p>
      </div>
    </section>
  </main>
